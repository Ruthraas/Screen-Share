...
import type { CaptureQuality } from "../services/captureClient";
import type { Route } from "../data/types";

export function Share({ navigate: _navigate }: { navigate: (route: Route) => void }) {
  const { selected, user } = useAccount();
  const { status, error, stream, start, stop } = useLocalCapture();
  const [pickerOpen, setPickerOpen] = useState(false);

  async function handleStart(sourceId: string, quality: CaptureQuality, audioEnabled: boolean) {
    setPickerOpen(false);
    await start(sourceId, quality, audioEnabled);
  }

  if (status === "active" && stream) {
    return (
      <section className="share-page">
        <ScreenViewer user={user} stream={stream} onMinimize={() => void stop()} onStreamEnded={() => void stop()} />
      </section>
    );
  }
  ...
  return (
    <section className="share-page">
      ...
      <CaptureSourcePicker open={pickerOpen} onClose={() => setPickerOpen(false)} onStart={(sourceId, quality, audioEnabled) => void handleStart(sourceId, quality, audioEnabled)} />
    </section>
  );
}
